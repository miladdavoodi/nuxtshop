...
And developed by  Reza Moghaddam : [  http://moghaddam24.ir  ]  v1.1
--------------------------------------------------------------------
...
if( ! function_exists('jstrftime') ){
    /*	F	*/
    function jstrftime($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='en'){
...
if( ! function_exists('todate') ) {
    function todate($date)
    {
        ...
        $date_gregorian = jalali_to_gregorian($date[0], $date[1], $date[2]);
        if ($date_gregorian[1] <= 9) $date_gregorian[1] = '0' . $date_gregorian[1];
        if ($date_gregorian[2] <= 9) $date_gregorian[2] = '0' . $date_gregorian[2];

        return $date_gregorian[0] . '/' . $date_gregorian[1] . '/' . $date_gregorian[2];
    }
}
...
if( ! function_exists('jstrtotime') ){
    function jstrtotime($time)
    {
        ...
        if( preg_match('/(.*\D)?(\d{4})[\/\-](\d{1,2})[\/\-](\d{1,2})[\s\-]+(\d{1,2}):(\d{1,2}):(\d{1,2})(\D.*)?/', $time, $timeArray) ){
            $finalTime = jmktime($timeArray[5], $timeArray[6], $timeArray[7], $timeArray[3], $timeArray[4], $timeArray[2]);
            $lastIndexTime = 7;
        }else if( preg_match('/(.*\D)?(\d{4})[\/\-](\d{1,2})[\/\-](\d{1,2})[\s\-]+(\d{1,2}):(\d{1,2})(\D.*)?/', $time, $timeArray) ){
            $finalTime = jmktime($timeArray[5], $timeArray[6], '00', $timeArray[3], $timeArray[4], $timeArray[2]);
            $lastIndexTime = 6;
        }else if( preg_match('/(.*\D)?(\d{1,2})[\/\-](\d{1,2})[\s\-]+(\d{1,2}):(\d{1,2}):(\d{1,2})(\D.*)?/', $time, $timeArray) ){
            $finalTime = jmktime($timeArray[4], $timeArray[5], $timeArray[6], $timeArray[2], $timeArray[3]);
            $lastIndexTime = 6;
        }else if( preg_match('/(.*\D)?(\d{1,2})[\/\-](\d{1,2})[\s\-]+(\d{1,2}):(\d{1,2})(\D.*)?/', $time, $timeArray) ){
            $finalTime = jmktime($timeArray[4], $timeArray[5], '00', $timeArray[2], $timeArray[3]);
            $lastIndexTime = 5;
        }else if( preg_match('/(.*\D)?(\d{4})[\/\-](\d{1,2})[\/\-](\d{1,2})(\D.*)?/', $time, $timeArray) ){
            $finalTime = jmktime('00', '00', '00', $timeArray[3], $timeArray[4], $timeArray[2]);
            $lastIndexTime = 4;
        }else if( preg_match('/(.*\D)?(\d{1,2}):(\d{1,2}):(\d{1,2})(\D.*)?/', $time, $timeArray) ){
            $finalTime = jmktime($timeArray[2], $timeArray[3], $timeArray[4]);
            $lastIndexTime = 4;
        }else if( preg_match('/(.*\D)?(\d{1,2})[\/\-](\d{1,2})(\D.*)?/', $time, $timeArray) ){
            $finalTime = jmktime('00', '00', '00', $timeArray[2], $timeArray[3]);
            $lastIndexTime = 3;
        }else if( preg_match('/(.*\D)?(\d{1,2}):(\d{1,2})(\D.*)?/', $time, $timeArray) ){
            $finalTime = jmktime($timeArray[2], $timeArray[3], '00');
            $lastIndexTime = 3;
        }

        $postFix = $lastIndexTime < ($FI = count($timeArray)-1) ? true : false;

        if(!empty($timeArray[1]) || $postFix) {
            $date = date("Y-m-d H:i:s", $finalTime);
            if($timeArray[0]){
                $date = $timeArray[1] . $date;
            }
            if($postFix){
                $date .= $timeArray[$FI];
            }
